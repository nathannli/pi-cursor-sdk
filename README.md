...
Config can also set non-secret defaults in `~/.pi/agent/cursor-sdk.json` or trusted `.pi/cursor-sdk.json`. A manually created standalone `.pi/cursor-sdk.json` is ignored unless `.pi/settings.json` or another Pi-recognized trust resource existed and the project was trusted at session start; after adding a trust resource, restart pi. `/cursor-runtime ... --save-project` requires that trusted project state and does not create Pi trust resources automatically.
