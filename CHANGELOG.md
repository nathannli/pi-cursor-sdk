...
- Update live smoke and visual validation requirements for pi 0.76.0 `--session-id`, Cursor SDK plan mode, deterministic native-replay-only card-category proof, direct parent-`PATH` command resolution in tmux-launched runners, debug-env isolation, and mandatory browser-rendered card/color inspection through the canonical offscreen PTY + ANSI + PNG + JSONL path.
