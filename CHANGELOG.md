...
- Seed Cursor SDK mode through `Agent.create({ mode })`, switch reused pooled agents with send-time `agent.send(..., { mode })`, and commit mode state only after successful finalization.
- Update live smoke and visual validation requirements for pi 0.76.0 `--session-id`, Cursor SDK plan mode, and mandatory card/color inspection.
