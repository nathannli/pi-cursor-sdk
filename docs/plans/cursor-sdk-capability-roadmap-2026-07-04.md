...
|      5 | Cloud runtime follow-through remains minimal.                                      | Explicit cloud runtime starts `Agent.create({ cloud })` after preflight, with fresh context by default, no pi bridge/local MCP/env forwarding, runtime-aware footer status through the canonical resolver, cloud agent names from the pi session title, and explicit Cursor-managed cloud environment selection. | Cloud repos/PR/artifacts/list/resume/usage APIs exist, but several are not yet integrated into pi UX.                                             | Keep local default preserved; add cloud reporting, artifact/usage display, resume policy, and any future remote pi-tool bridge only behind explicit product decisions. |
