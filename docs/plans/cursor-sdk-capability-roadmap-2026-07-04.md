...
| Same-session process restart                  | **Validated**                      | Platform suite `cursor-local-resume-restart` and focused `npm run smoke:local-resume` prove a recorded local SDK agent resumes across a pi process restart when `PI_CURSOR_LOCAL_RESUME=1`. |
