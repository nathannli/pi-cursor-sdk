...
**Implemented**: `src/cursor-config.ts` provides the effective-config resolver with ordinary precedence, safety caps, fast-default migration, cloud/runtime/env configuration, first-use cloud acknowledgement, explicit save destinations, and trust-gated project config loading. Explicit cloud runtime now starts `Agent.create({ cloud })` after preflight, with fresh context by default and no pi bridge, inline MCP, or pi env forwarding. A minimal opt-in `npm run smoke:cloud` lane validates that path. Status footer shows local/cloud runtime using the canonical resolver path. Cloud agents are named from the pi session title when available. Explicit Cursor-managed cloud environment selection is wired without local env forwarding. Bounded cloud completion telemetry streams display-only agent/run IDs, branch/PR metadata, passive artifacts, and raw usage when available without persisting it into transcript content or feeding cloud usage into pi accounting; immediately after Cloud `Agent.create()`, a branch-local pi lifecycle entry is fsynced in Pi JSONL before anchoring each fsynced, newline-framed sidecar record keyed by stable pi session ID (with a one-time branch-claimable orphan marker for fileless first turns), the returned run ID is recorded before abort handling or waiting, malformed lines are isolated from later valid records, and successful reports enrich the ledger with branch/PR metadata; explicit `/cursor-cloud list|archive|delete` commands accept exact recorded `bc-` IDs only and use durable intent/result records around irreversible SDK mutations. Guarded local branch-scoped SDK resume is enabled by default for local runtime, with opt-out through `local.resume: false`, `--cursor-no-local-resume`, or `PI_CURSOR_LOCAL_RESUME=0`; cloud resume remains deferred. Current follow-up statuses: env forwarding, SDK agents, URL display, inline cloud MCP, automatic local force, and remote Pi bridge are classified below.
...
| Cloud branch/ref     | `--cursor-cloud-branch`, `PI_CURSOR_CLOUD_BRANCH`                                        | `cloud.branch`                      | explicit override                  | Requires an explicit cloud repo because SDK `startingRef` exists only on `cloud.repos` entries. Reserved for CLI/env/user/session override; do not infer or project-save. |
...
| Local resume         | `--cursor-local-resume`, `--cursor-no-local-resume`, `PI_CURSOR_LOCAL_RESUME`          | `local.resume`                      | ordinary runtime feature          | Defaults on for local runtime. Local only; stores agent IDs in pi session custom entries, not config. Use `PI_CURSOR_LOCAL_RESUME=0` or `local.resume: false` to opt out. |
...
| Cloud reporting | **Implemented** | Cloud direct runs stream bounded display-only agent/run IDs, pushed branch/repository/PR metadata, passive artifact paths, and raw usage when available. The telemetry is stream-only and raw cloud usage stays out of pi AssistantMessage usage, context occupancy, compaction, and cost totals. |

### Open capability decisions

| Impact | Area | Current code | SDK capability | Status / next condition |
| -----: | ---- | ------------ | -------------- | ----------------------- |
|      1 | Pi tool bridge vs SDK `customTools` | `src/cursor-pi-tool-bridge-run.ts` starts an HTTP MCP endpoint; `src/cursor-session-agent.ts` passes `mcpServers` into `Agent.create`. | `LocalAgentOptions.customTools` / `LocalSendOptions.customTools` expose caller functions through the SDK's synthetic `custom-user-tools` MCP server. | **Needs SDK/API change**. Loopback MCP is the sole implemented transport. Next condition: SDK cancellation/deadline support, or a pi-owned adapter that supplies abort signals, timeouts, child cleanup, diagnostics, permissions, and platform-smoke parity. |
|      2 | Branch-scoped `Agent.resume()` | Local runtime uses in-memory pooling plus guarded session-custom-entry resume by default; cloud runtime still creates a fresh cloud agent each turn. | `Agent.resume(agentId)` can reattach to local/cloud persisted agent state after process restart. | **Implemented** for default-on local resume. Cloud resume remains deferred pending broader live smoke and product decision. |
...
|      5 | Cloud reporting + lifecycle | Explicit cloud runtime starts `Agent.create({ cloud })` after preflight, with fresh context by default, no pi bridge/local MCP/env forwarding, runtime-aware footer status through the canonical resolver, cloud agent names from the pi session title, explicit Cursor-managed cloud environment selection, a persisted pi session with branch-bound fsynced agent intent before send, returned run-ID persistence before abort/wait, a mirrored pi branch ledger, and successful-report enrichment afterward. | Cloud agent/run IDs, branch/PR metadata, artifacts, archive/delete, and raw usage endpoints exist. | **Implemented** for bounded display telemetry plus explicit `/cursor-cloud list`, `/cursor-cloud archive <bc-agentId>`, and `/cursor-cloud delete <bc-agentId> --yes` for recorded `bc-` IDs only. No cloud resume/default-on, auto-cleanup, bulk delete, raw delete filters, or local store sweeping. URL display stays deferred by product decision. |
...
Resume is desirable for both local and cloud agents, and the evidence sweep resolved the SDK contract enough for guarded local default-on implementation. The current implementation is intentionally local-only. It respects pi's session tree semantics by restoring only records found on the active pi branch path and still must not persist one SDK `agentId` per pi session file for all branches.
...
- Pass the current pi model on every `agent.send()` and the current loopback `mcpServers` to `Agent.resume(...options)`.
- Missing/deleted/unavailable local resume falls back to create + bootstrap with one display-only continuity note.
- Tests cover default-on config behavior, opt-out behavior, matching branch restore, copied session-entry rejection, tree/compaction clearing, compaction generation mismatch, model/API/tool-surface pool-key mismatch, resume failure fallback, custom-entry persistence, and a real hard-killed child proving startup-persisted user prompts invalidate older handles.

Remaining before cloud resume:
...
- Fork/clone, tool-surface mismatch, abort/interrupted-turn, tree navigation, copied session switch, missing-agent fallback, compaction boundary, default/opt-out proof, and recorded-ID-only cleanup now have focused smoke lanes and are part of the required platform suite list.
- Tree and crash safety were tightened after live proof found that direct navigation to an earlier `cursor-sdk-agent-resume` custom entry could reuse a future-seeing SDK agent. Old handles are rejected when the same session/pool/agent has a newer resume handle anywhere in the session file, and they cannot span a user entry already present at process startup or selected by tree navigation because the prior process may already have submitted it.
...
- **Validated**: local `mcpServers` are not persisted. Without resupply, resumed runs can finish with assistant-visible tool failure such as `MCP server does not exist`; resupplying the current loopback MCP server through `Agent.resume(...options)` restores Pi tools.
...
- Same active branch/path after process restart: resume the recorded SDK agent when the session file/id and branch/path match, then bootstrap the current pi transcript once instead of trusting persisted incremental-send state. This retains Cursor-native state while making pi-visible continuity independent of checkpoint timing; later in-process turns remain incremental.
...
- `/tree` to the latest unsuperseded branch/path with a matching recorded SDK agent: resume that agent.
- `/tree` to a branch/path with no matching SDK agent: create a new SDK agent and bootstrap from pi's active context.
- `/tree` moving back to an earlier point must not reuse an SDK agent that has seen messages beyond the selected leaf. If the same session/pool/agent has a newer resume handle anywhere in the session file, treat the older handle as superseded and create a new SDK agent.
...
- Every new branch agent, compaction boundary, tool-surface fallback, or periodic rebootstrap can mint another SDK agent. New resume entries record replaced predecessor agent IDs as cleanup candidates when a different local agent supersedes the prior branch handle; cleanup protects agents still resumable from any session-tree branch.
...
- **Validated**: SDK local-store delete filters treat omitted or empty IDs as match-all. Cleanup verifies and fsyncs an exact `cursor-sdk-agent-cleanup` intent in the Pi session JSONL before each SDK `Agent.delete(agentId, { cwd })` batch, then verifies and fsyncs the result; unresolved intent plus a conservative current-process marker blocks automatic retry when result durability fails.
...
- Branch-scoped local resume is default-on for local runtime.
- Create/bootstrap remains the fallback for opt-out, missing/unavailable SDK state, identity mismatch, tool-surface mismatch, tree safety, compaction boundaries, abort recovery, and any rejected stale handle.

Required resume probes/tests before flipping defaults:

| Probe                                         | Status                             | Notes                                                                                                                                                                      |
| --------------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool re-supply without accidental persistence | **Validated**                      | Missing MCP resupply fails; the current loopback `mcpServers` supplied to `Agent.resume()` restores exposed Pi tools.                                                       |
...
| Same-session process restart                  | **Validated**                      | Platform suite `cursor-local-resume-restart` and focused `npm run smoke:local-resume` prove a recorded local SDK agent resumes across a pi process restart with the built-in default enabled. |
| Fork/clone isolation                          | **Pi policy**                      | `cursor-local-resume-safety` / `npm run smoke:local-resume:safety` prove cloned-session copied resume entries and fork-before-future branches do not reuse the original local `agent-*`, and the earlier fork does not reveal the future token. |
| Tool-surface mismatch                         | **Pi policy**                      | `cursor-local-resume-tool-surface` / `npm run smoke:local-resume:tool-surface` prove a bridge builtin tool-surface change rejects the old local resume handle, creates a bridge run, uses a new local `agent-*`, and persists a new resume pool key. |
| Abort/interrupted turn                        | **Pi policy**                      | `cursor-local-resume-abort` / `npm run smoke:local-resume:abort` prove an interrupted long-running bridge turn does not append a new resume handle, and the next same-surface restart does not reuse the pre-abort local `agent-*`. |
| Tree navigation                               | **Pi policy**                      | `cursor-local-resume-tree` / `npm run smoke:local-resume:tree` prove earlier assistant and earlier resume-entry tree targets reject the future-seeing SDK agent and do not leak the future token. |
| Session copy/switch                           | **Pi policy**                      | `cursor-local-resume-copy-switch` / `npm run smoke:local-resume:copy-switch` prove a copied session file carrying resume entries rejects the copied handle while bootstrapping from transcript. |
...
| Default-on / opt-out proof                    | **Pi policy**                      | `cursor-local-resume-default-dry-run` / `npm run smoke:local-resume:default-dry-run` prove the built-in default resumes, and `PI_CURSOR_LOCAL_RESUME=0` opts out. |
...
- Preserve pi compaction semantics: `totalTokens = input + output + cacheRead + cacheWrite`.
...
- Use real `turn-ended` usage when available, when it fits the selected model window, and recompute pi `total = input + output + cacheRead + cacheWrite`.
...
- dirty or unpushed local-only state exists: current interactive and non-interactive runs fail every time unless explicitly allowed for that send/session; fingerprinted interactive warn-once/status behavior is deferred, and dirty/unpushed detection is pi-owned because Cursor Cloud cannot see local-only work;
...
Repo, branch, env, cleanup, and lifecycle setup belong to Cursor Cloud by default. Pi may pass an explicit repo override alone or an explicit branch/ref only together with an explicit repo; it should not infer, prompt for, or project-save those choices in the initial cloud runtime. SDK/API errors remain authoritative for repo/provider/branch access.
...
Focused follow-up prompts for context bootstrap, dirty/unpushed local state, or explicit direct-push/local-state overrides are deferred. Current interactive and non-interactive runs fail every time unless an explicit allow is active. If interactive dirty-state prompting is added later, warn once per dirty/unpushed-state fingerprint and degrade unchanged state to a footer/status indicator.
...
- **Pi policy**: do not set `startingRef` by inference in the initial runtime; pass it only when both repo and branch/ref come from explicit CLI/env/user/session input because the SDK exposes `startingRef` only on `cloud.repos` entries. Do not set `workOnCurrentBranch: true` unless the user explicitly opts into pushing commits to that existing branch. **Validated**: explicit `workOnCurrentBranch: true` direct-pushed to the selected branch and produced no PR URL.
...
- Do not infer or project-save an active branch/ref by default. If both repo and branch/ref are supplied explicitly, pass the branch/ref as `startingRef` on that repo entry; reject branch/ref without repo because the SDK has no top-level `startingRef`. Otherwise let Cursor Cloud/SDK defaults decide or fail with an SDK/API error.
- Support `--cursor-cloud-branch` only with an explicit `--cursor-cloud-repo`, plus a separate direct-push flag if `workOnCurrentBranch` is exposed. Direct push must remain one-shot/user/session scoped; no project default in the initial runtime.
- Before a cloud send, detect local uncommitted changes and commits not pushed to the selected remote/ref when pi can observe them. **Implemented**: interactive and non-interactive runs currently fail every time unless an explicit allow is active. Fingerprinted interactive warn-once/status behavior remains deferred.
...
- **Implemented initial slice**: cloud starts fresh unless a user/session/CLI/env option explicitly selects bootstrap. The first-use disclosure explains both behaviors, but a per-switch interactive fresh/bootstrap chooser is deferred.
- The handoff preference can be remembered in user/session state to reduce friction, but the default stays fresh. Project config cannot save `bootstrap` in the initial runtime.
- Non-interactive cloud switches start fresh unless a user/session/CLI/env option explicitly allows sending current pi context to cloud.
...
- **Validated**: historical read-only live probes from 2026-07-06: `Cursor.me()` OK with user-scoped key; `Cursor.models.list()` returned 32 models; `Cursor.repositories.list()` returned 191 repos and listed this repo; `Agent.list({ runtime: 'cloud' })` OK. The authenticated 2026-07-09 release catalog has 34 models. Bad key → 401 `Invalid User API Key`.
...
- Leave cloud agents alive/archiveable after normal pi exit. Do not add automatic cleanup, exit prompts, bulk operations, or raw filters. Explicit `/cursor-cloud list`, `/cursor-cloud archive <bc-agentId>`, and `/cursor-cloud delete <bc-agentId> --yes` commands operate only on exact recorded cloud agent IDs; Cursor UI remains available for broader lifecycle management.
...
- When Cursor pushes a branch or opens a PR, show the pushed branch name, repository, and PR URL if present so users can find the work without hunting through the dashboard.
...
| Repo / branch / direct push | No inferred defaults in pi. Explicit repo may be passed alone; explicit branch/ref requires explicit repo because SDK `startingRef` exists only on `cloud.repos` entries. Otherwise Cursor Cloud/SDK owns defaults or errors. | `--cursor-cloud-repo` stays a power-user override; `--cursor-cloud-branch` is accepted only with it. Neither is a project default. Direct push stays one-shot/user/session only. |
| Dirty/unpushed local state | **Implemented initial slice**: interactive and non-interactive runs fail every time unless explicitly allowed. Fingerprinted interactive warn-once/status UX is deferred. | Avoid repeated click-through warnings if the deferred interactive UX is added. |
...
| Lifecycle | Leave cloud agents alive/archiveable after normal pi exit; explicit recorded-ID-only `/cursor-cloud list|archive|delete` commands are implemented. | No automatic or bulk cleanup, exit prompt, raw filter, or unrecorded-ID mutation. |
...
- **Implemented**: guarded local branch-scoped SDK resume is default-on for local runtime with opt-out via `local.resume: false`, `--cursor-no-local-resume`, or `PI_CURSOR_LOCAL_RESUME=0`; cloud resume remains deferred.
...
- **Needs SDK/API change**: `customTools` remains deferred until cancellation/deadline support exists or pi owns a full adapter; loopback MCP is the sole implemented Pi-tool transport.
...
| Local resume tool re-supply                  | **Validated**                      | Local `mcpServers` are not persisted. Missing resupply can finish as assistant-visible tool failure; supplying the current loopback MCP server to `Agent.resume()` restores exposed Pi tools.                                                                                                                    |
...
| Cloud repo entry with explicit `startingRef` | **Validated**                      | Separate `cursor/...` branch + PR, not direct push; the SDK exposes `startingRef` only on `cloud.repos` entries.                                                                                                                                                                                                                                                              |
...
- explicit repo entry with `startingRef` and default `workOnCurrentBranch: false` → Cursor creates a separate branch/PR (**Validated**);
...
- `docs/cursor-tool-surfaces.md` for MCP/customTools/cloud Pi-tool availability — **current policy documented: loopback MCP is the sole implemented transport, customTools needs SDK/API change before migration, cloud gets no local Pi tools**.
...
- Live local compaction boundary evidence captured 2026-07-08 in `docs/evidence/cursor-local-compaction-boundary-2026-07-08.md` and sanitized report excerpt `docs/evidence/cursor-local-compaction-boundary-2026-07-08.report.json`. The run used `cursor/composer-2-5:slow`, compacted a three-turn session (`tokensBefore: 113244`, `estimatedTokensAfter: 40260`), wrote compaction entry `5c65d5c6`, created post-compaction agent `agent-b5e5e885-9c63-4415-9593-575418449607` instead of reusing pre-compaction agent `agent-9f5c78fb-458c-4225-9976-a95b22806221`, recalled `COMPACT_BOUNDARY_1783484227852`, and resumed the generation-1 agent on the next restart. Full session snapshots remain local gitignored artifacts because they contain large filler prompts.
...
- Cloud API behavior anchors:
  ...
  - `workOnCurrentBranch` defaults false, so a repo entry with `startingRef` normally creates a separate Cursor branch.
