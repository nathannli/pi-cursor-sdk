...
- Same-session restart proof is now covered by `cursor-local-resume-restart` in `npm run smoke:platform:all` and by focused `npm run smoke:local-resume`. Focused `npm run smoke:local-resume:safety` covers clone rejection and fork-before-future no-leak locally. Focused `npm run smoke:local-resume:tool-surface` covers bridge builtin tool-surface mismatch locally. Default-on still needs broader tree/import/abort/platform coverage.
...
- Broader live smoke must prove tree, import/session switch, abort, platform tool-surface coverage, and resume failure fallback.
