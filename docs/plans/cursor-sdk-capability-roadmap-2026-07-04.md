...
- Same-session restart proof is now covered by `cursor-local-resume-restart` in `npm run smoke:platform:all` and by focused `npm run smoke:local-resume`. Focused `npm run smoke:local-resume:safety` covers clone rejection and fork-before-future no-leak locally. Focused `npm run smoke:local-resume:tool-surface` covers bridge builtin tool-surface mismatch locally. Focused `npm run smoke:local-resume:abort` covers interrupted bridge-turn persistence locally. Default-on still needs broader tree/import/platform coverage.
...
- Broader live smoke must prove tree, import/session switch, platform abort/tool-surface coverage, and resume failure fallback.
...
| Fork/clone isolation                          | **Pi policy**                      | Focused `npm run smoke:local-resume:safety` proves cloned-session copied resume entries and fork-before-future branches do not reuse the original local `agent-*`, and the earlier fork does not reveal the future token. More tree/import/platform smoke remains before default-on. |
| Tool-surface mismatch                         | **Pi policy**                      | Focused `npm run smoke:local-resume:tool-surface` proves a bridge builtin tool-surface change rejects the old local resume handle, creates a bridge run, uses a new local `agent-*`, and persists a new resume pool key. More platform coverage remains before default-on. |
| Abort/interrupted turn                        | **Pi policy**                      | Focused `npm run smoke:local-resume:abort` proves an interrupted long-running bridge turn does not append a new resume handle, and the next same-surface restart does not reuse the pre-abort local `agent-*`. More platform coverage remains before default-on. |
