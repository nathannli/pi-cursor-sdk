...
- Same-session restart proof is now covered by `cursor-local-resume-restart` in `npm run smoke:platform:all` and by focused `npm run smoke:local-resume`. Focused `npm run smoke:local-resume:safety` covers clone rejection and fork-before-future no-leak locally. Default-on still needs broader tree/import/abort/tool-surface platform coverage.
